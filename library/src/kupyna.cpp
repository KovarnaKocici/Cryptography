...
int Kupyna::ProofOfWork(uint8_t* in, size_t blocks, size_t zeros_count, uint8_t* out)
{
	//if (zeros_count >= 64) {
	//	return -1;
	//}
	...
	//		i++;
	//	}
	//} while (CMPBits(out[message_diggest_bsize / sizeof(uint8_t) - 1], 0, zeros_count) == 0);

	return 0;
}

int Kupyna::CMPBits(uint8_t v1, uint8_t v2, size_t count)
{
