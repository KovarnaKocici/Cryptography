...
Salsa20::Salsa20(const uint8_t key[], const uint8_t iv[IV_LENGTH])
{
	keySetup(key);
	ivSetup(iv);
}

void Salsa20::keySetup(const uint8_t key[])
{
...
void Salsa20::ivSetup(const uint8_t iv[])
{
	if (iv == 0)
	{
...
void Salsa20::wordToByte(uint32_t input[])
{
	uint32_t x[16];
	int i;
...
void Salsa20::encrypt(uint8_t m[], uint32_t bytes)
{
	uint32_t i;
...
void Salsa20::decrypt(uint8_t m[], const uint32_t bytes)
{
	encrypt(m, bytes);
}